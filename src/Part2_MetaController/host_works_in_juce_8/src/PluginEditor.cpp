...
void PluginHostEditor::buttonClicked(juce::Button* btn)
{
    ...
    if (btn == &loadPluginButton){
        // close any existing plugin UI before loading a new plugin
        pluginWindow.reset();
        # ifdef JUCE_LINUX
    ...
    if (btn == &showPluginUIButton){
        if (! audioProcessor.hasLoadedPlugin()) {
            DBG("showPluginUIButton no plugin loaded");
            return;
        }
        ...
        auto pluginEditor = audioProcessor.createHostedPluginEditor();
        if (pluginEditor == nullptr){
            DBG("showPluginUIButton failed to create plugin editor");
            return;
        }

        DBG("HosterPluginAudioProcessorEditor::showPluginGUI creating new gui");
        pluginWindow = std::make_unique<PluginWindow>(std::move(pluginEditor));
