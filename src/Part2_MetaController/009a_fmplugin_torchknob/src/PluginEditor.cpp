...
void FMPluginEditor::resized()
{
    ...
    superKnob.setBounds(colWidth, rowHeight*row, colWidth, rowHeight*2);
    row +=2;
 ...
 void FMPluginEditor::sliderValueChanged (juce::Slider *slider)
{
    ...
    if (slider == &superKnob){
        ...
        // modDepthSlider.setValue(nn_outs[0] * modDepthSlider.getMaxValue());
        // modIndexSlider.setValue(nn_outs[1] * modIndexSlider.getMaxValue());
        torch::Tensor in = torch::empty({1, 1});
        in[0][0] = superKnob.getValue();
        torch::Tensor out = linear(in);
        std::cout << out << std::endl;
        modDepthSlider.setValue(out[0][0].item<float>());
        modIndexSlider.setValue(out[0][1].item<float>());
