...
#endif
{
    ...
    auto& waveshaper = processorChain.template get<ws_index>(); 
    ...
    // high pass IIR
    // auto& filter = processorChain.get<iir_index>();
    // filter.coefficients = juce::dsp::IIR::Coefficients<float>
    //                           ::makeHighPass(44100, 200);
    // impulse response convolver
    auto& convolution = processorChain.template get<conv_index>();
    ...
    juce::File impFile{"/home/matthewyk/src/ai-enhanced-audio/ai-audio-book-code/src/Part4_NeuralFX/audio/orange_minicab_impulse.wav"};
...
void TestPluginAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    // auto& filter = processorChain.get<iir_index>();
    // filter.coefficients = juce::dsp::IIR::Coefficients<float>
    //                           ::makeHighPass(getSampleRate(), 200);

    // setup the IIR filter
    auto& filter = processorChain.template get<iir_index>();
    filter.state = FilterCoefs::makeHighPass (getSampleRate(), 200.0f);
