...
public:
    ...
    /** whether a plugin node is loaded into the graph */
    bool hasLoadedPlugin() const noexcept { return pluginNode != nullptr; }

    /** create an editor for the hosted plugin (generic fallback when needed) */
    std::unique_ptr<juce::AudioProcessorEditor> createHostedPluginEditor();
