...
{
    if (pluginNode){
      return pluginNode->getProcessor()->createEditorIfNeeded();
    }
    else{
      return nullptr;
    }
...
void PluginHostProcessor::printPluginParameters()
{
    int pCount{0};

  if (pluginNode){
  juce::Array<juce::AudioProcessorParameter *> params = pluginNode->getProcessor()->getParameters();
    for (const juce::AudioProcessorParameter* p : params ){
      if (! p->getName(100).contains("MIDI CC ")){
        DBG("p: " << pCount << ":" << p->getName(10) << ": " << p->getValue());
        pCount ++; 
...
int PluginHostProcessor::countValidParameters()
{
  int pCount{0};
...
std::vector<float> PluginHostProcessor::getHostedPluginParamValues()
{
