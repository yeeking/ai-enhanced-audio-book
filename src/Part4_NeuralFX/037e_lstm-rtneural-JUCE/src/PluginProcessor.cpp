...
void TestPluginAudioProcessor::setupModel(RTLSTMModel32& model, std::string jsonFile){
    int lstm_units = 32; 
    ...
    if (json_lstm_size != lstm_units){
        std::cout << "Model wants " << lstm_units 
                  << " lstm units but JSON file specifies " 
                  << json_lstm_size << std::endl;
        throw(std::exception());
    }
